...
Процедура СоздатьПользователяПоПараметрам(ПараметрыПользователя)
	...
	СписокЗаполняемыхПолей = "ПоказыватьВСпискеВыбора, 
								|ПотребоватьСменуПароляПриВходе, 
								|ЗапрещеноИзменятьПароль, 
								|ЗапрещеноВосстанавливатьПароль, 
								|АутентификацияСтандартная, 
...
//	КонецЕсли;
//	
//	ПользовательИБ.АутентификацияОС			= ПараметрыПользователя.АутентификацияОС;
//	ПользовательИБ.АутентификацияСтандартная	= ПараметрыПользователя.АутентификацияСтандартная;
//	
//	ПользовательИБ.АдресЭлектроннойПочты = ПараметрыПользователя.АдресЭлектроннойПочты;
	
	ЗаполнитьЗначенияСвойств(ПользовательИБ, ПараметрыПользователя, СписокЗаполняемыхПолей);
	
	Если ЗначениеЗаполнено(ПараметрыПользователя.Пароль) Тогда
		ПользовательИБ.Пароль = ПараметрыПользователя.Пароль;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПараметрыПользователя.СохраняемоеЗначениеПароля) Тогда
		ПользовательИБ.СохраняемоеЗначениеПароля = ПараметрыПользователя.СохраняемоеЗначениеПароля;
	КонецЕсли;
