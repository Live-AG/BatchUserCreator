...
Процедура ЗагрузитьПользователей(ДанныеПользователей) Экспорт
	
	Для Каждого ПараметрыПользователя Из ДанныеПользователей Цикл
		
		СоздатьПолизователяПоПараметрам(ПараметрыПользователя);	
		
	КонецЦикла;
	
КонецПроцедуры

Процедура СоздатьПолизователяПоПараметрам(ПараметрыПользователя, ЗаполнятьКомментарий = Ложь)
...
Функция ПолучитьДанныеИзФайла(Знач ПутьКДанным) Экспорт
	
	ЧтениеТекста = Новый ЧтениеТекста(ПутьКДанным, КодировкаТекста.UTF8, Символы.ВК + Символы.ПС, Символы.ПС);
