...
&НаКлиенте
Функция ПолучитьПутьКВерсииПлатформы(ВерсияПлатформы)
	...
	ИначеЕсли МассивКаталогов.Количество() = 1 Тогда
		Возврат КаталогУстановкиПлатформы + МассивКаталогов[0].Имя + ПолучитьРазделительПутиКлиента();
	Иначе
	...
	Возврат КаталогУстановкиПлатформы + ТекущаяВерсияПлатформы + ПолучитьРазделительПутиКлиента();;
...
Процедура ВыполнитьЗапускОбработки(НаборБазДляЗапуска)
	...
	ШаблонСтрокиЗапуска = "%1 ENTERPRISE /IBConnectionString %2 WA+ /C ""Загрузить %3"" /Execute ""%4""  /DisableStartupMessages ";
	...
	Для Каждого ПараметрыЭлемента Из НаборБазДляЗапуска Цикл
		...
		Если ПараметрыЭлемента.Свойство("Version") 
			...
			ПутьКПрограмме = ПолучитьПутьКВерсииПлатформы(ПараметрыЭлемента.Version);
		Иначе
			ПутьКПрограмме = ПолучитьПутьКВерсииПлатформы(ВерсияПлатформы);
		КонецЕсли;
		...
		СтрокаПодключения = СтрЗаменить(ПараметрыЭлемента.Connect, ";", "");
