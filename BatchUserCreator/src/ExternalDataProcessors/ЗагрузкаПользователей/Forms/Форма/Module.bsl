...
&НаКлиенте
Асинх Процедура ПрочитатьДанныеИзФайла(Команда)
...
Функция ПолучитьДанныеИзФайла(ПутьКФайлу)
	...
	СоставПолей.Добавить("ПрофилиРолей");
...
Процедура ЗаполнитьПараметрыСтроки(ТаблицаДанных, МассивЗначений, НомерСтроки)
	...
	Для ИндексПараметра = 0 По МассивЗначений.ВГраница() Цикл
		ТаблицаДанных.Область(НомерСтроки, ИндексПараметра + 1, НомерСтроки, ИндексПараметра + 1)
									.Текст = МассивЗначений[ИндексПараметра];
	КонецЦикла;
