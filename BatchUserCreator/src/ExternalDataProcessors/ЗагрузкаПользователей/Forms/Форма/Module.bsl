...
Функция ПолучитьДанныеИзФайла(ПутьКФайлу)
	...
	СоставПолей.Добавить("АдресЭлектроннойПочты");
...
Функция ЗаполнитьСлучайныхПользователейНаСервере()
	...
	Для Каждого ФИОПользователя Из НаборФИОПользователей Цикл
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.ПолноеИмя, 
											СчетчикСтрок, Колонки.ПолноеИмя).Текст = ФИОПользователя;
		...
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.Пароль, 
											СчетчикСтрок, Колонки.Пароль).Текст = НовыйПароль;
		
		
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.ПраваДоступа, 
											СчетчикСтрок, Колонки.ПраваДоступа).Текст = ДанныеПравДоступа;
											
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.Комментарий, 
											СчетчикСтрок, Колонки.Комментарий).Текст = Комментарий;
...
Процедура ЗаполнитьПараметрыВСтроке(НомерСтроки, МассивУникальныхИмен);
	...
	СтруктураЗначений.ПолноеИмя = УдалитьСимволы(ОбластьПолноеИмя.Текст);
	...
	//ва  = СтрЗаменить(ОбластьПолноеИмя.Текст, "а", "ва");
	СтруктураИмени = ЧастиИмени(СтруктураЗначений.ПолноеИмя);
	...
	СтруктураЗначений.АдресЭлектроннойПочты	= СтрШаблон(ШаблонПочты,	ИмяЛатиницей);
...
Процедура ЗаполнитьТекстЯчеек(НомерСтроки, СтруктураЗначений)
	...
	СписокПолей = "Пароль, ПраваДоступа, Комментарий";
	...
	Для Каждого КолонкаТаблицы Из Колонки Цикл
		...
		Если ПроверяемыеПоля.Найти(КолонкаТаблицы.Ключ) <> Неопределено 
			И ПустаяСтрока(ОбластьЗначения.Текст) Тогда
			Продолжить;
		КонецЕсли;
