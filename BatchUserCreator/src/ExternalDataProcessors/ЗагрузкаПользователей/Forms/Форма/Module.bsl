...
Функция ЗаполнитьСлучайныхПользователейНаСервере(НастройкиЗаполнения)
	...
	СчетчикСтрок = 1;
	Для Каждого ОписаниеПользователя Из НаборНовыхПользователей Цикл
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.ПолноеИмя, 
											СчетчикСтрок, Колонки.ПолноеИмя).Текст = ОписаниеПользователя.ПолноеИмя;
		
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.Пароль, 
											СчетчикСтрок, Колонки.Пароль).Текст = ОписаниеПользователя.Пароль;
		
		Если ВариантУстановкиДоступа = 0 Тогда
			ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.ПраваДоступа, 
											СчетчикСтрок, Колонки.ПраваДоступа).Текст = ОписаниеПользователя.ПользовательОснование;
		ИначеЕсли ВариантУстановкиДоступа = 1 Тогда
			ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.ПраваДоступа, 
											СчетчикСтрок, Колонки.ПраваДоступа).Текст = ОписаниеПользователя.ГруппыДоступа;
		КонецЕсли;								
											
											
		ДополнениеТабличногоДокумента.Область(СчетчикСтрок, Колонки.Комментарий, 
											СчетчикСтрок, Колонки.Комментарий).Текст = ОписаниеПользователя.Комментарий;
