...
Асинх Процедура ЭкспортироватьВФайл(Команда)
	...
	ЗаписатьДанныеВJSON(РезультатВыбора[0], ДанныеПользователей, Настройки);
...
Процедура ЗагрузитьВоВнешниеБазы(Команда)
	...
	ОткрытьФорму("ВнешняяОбработка.ЗагрузкаПользователей.Форма.ФормаСпискаИБ");
...
Процедура ОбработатьПакетныйЗапуск(Отказ)
	...
	ИначеЕсли СтрНайти(ПараметрЗапуска, "Генерировать", , 1) Тогда
		...
		ДанныеПользователей = ЗаполнитьПользователейТекущейБазыНаСервере();
		НастройкиЗаполнения = ПолучитьНастройки();
		НастройкиЗаполнения.ВариантУстановкиДоступа	= 1;
		НастройкиЗаполнения.ПарольХранимоеЗначение		= 1;
		
		ЗаписатьДанныеВJSON(ПутьКФайлу, ДанныеПользователей, НастройкиЗаполнения);
...
&НаКлиенте
Процедура ЗаписатьДанныеВJSON(ПутьКФайлу, ДанныеПользователей, Настройки)
